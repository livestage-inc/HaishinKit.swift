...
open class HTTPStream: NetStream {
    ...
    #if os(iOS) || os(macOS)
    override open func attachCamera(_ camera: AVCaptureDevice?, onCompletion:(()->Void)? = nil, onError: ((NSError) -> Void)? = nil) {
        if camera == nil {
            tsWriter.expectedMedias.remove(.video)
        } else {
            tsWriter.expectedMedias.insert(.video)
        }
        super.attachCamera(camera, onCompletion: onCompletion, onError: onError)
    }
